...
#'
#' @examples
#' test.cab <- convert.all.bins.mm10(DNA.mat, RNA.mat, n.cores=10)
#'
#' test.cab.uber <- test.cab$RNAadj
#' @export
convert.all.bins.mm10 <- function(DNA.mat, RNA.mat, n.cores){
##make list obj for each window
         ...
         start <- c(0, end[-length(end)])
          ls.all <- list()
          for(i in 1:nrow(DNA)){
          sub.anno <- full.anno.mm10[which(full.anno.mm10$chromosome_name==DNA$chrom[i]),]
